...
def _write_summary(run_dir: Path, login_df: pd.DataFrame, checkout_df: pd.DataFrame, split_boundary: pd.Timestamp, holdout_users: set[str]) -> None:
    reports_dir(run_dir).mkdir(parents=True, exist_ok=True)

    lines: list[str] = []
    lines.append(f"# Inkswarm DetectLab — Run Summary")
    lines.append("")
    ...
    if not login_df.empty:
        attack_any = ~login_df.get("label_benign", True)
    ...
    if not checkout_df.empty:
        adverse = (checkout_df["checkout_result"] != "success") if "checkout_result" in checkout_df.columns else pd.Series(False, index=checkout_df.index)
    ...
    lines.append(f"- holdout users: {len(holdout_users):,}")
    lines.append("")
...
def generate_raw(cfg: AppConfig, run_id: str | None = None) -> tuple[Path, dict[str, Any]]:
    ...
    lp, lfmt = write_auto(login_df, raw_table_basepath(rdir, "login_attempt"))
    cp, cfmt = write_auto(checkout_df, raw_table_basepath(rdir, "checkout_attempt"))

    artifacts["raw/login_attempt"] = {
        "path": str(lp.relative_to(cfg.paths.runs_dir)),
        "format": lfmt,
        "rows": int(len(login_df)),
        "content_hash": stable_hash_df(login_df, sort_keys=cfg.dataset.build.canonical_sort_keys),
    }
    artifacts["raw/checkout_attempt"] = {
        "path": str(cp.relative_to(cfg.paths.runs_dir)),
        "format": cfmt,
        "rows": int(len(checkout_df)),
        "content_hash": stable_hash_df(checkout_df, sort_keys=cfg.dataset.build.canonical_sort_keys),
    }
...
def build_dataset(cfg: AppConfig, run_id: str) -> tuple[Path, dict[str, Any]]:
    ...
    # build splits for each event table independently (but same holdout user logic is derived per table).
    # If we want shared holdout across tables, we can add CC later.
    ...
    for split_name, sdf in [("train", splits_login.train), ("time_eval", splits_login.time_eval), ("user_holdout", splits_login.user_holdout)]:
        p, fmt = write_auto(sdf, dataset_split_basepath(rdir, "login_attempt", split_name))
        artifacts[f"dataset/login_attempt/{split_name}"] = {
            "path": str(p.relative_to(cfg.paths.runs_dir)),
            "format": fmt,
            "rows": int(len(sdf)),
            "content_hash": stable_hash_df(sdf, sort_keys=split_cfg.canonical_sort_keys),
        }
    for split_name, sdf in [("train", splits_checkout.train), ("time_eval", splits_checkout.time_eval), ("user_holdout", splits_checkout.user_holdout)]:
        p, fmt = write_auto(sdf, dataset_split_basepath(rdir, "checkout_attempt", split_name))
        artifacts[f"dataset/checkout_attempt/{split_name}"] = {
            "path": str(p.relative_to(cfg.paths.runs_dir)),
            "format": fmt,
            "rows": int(len(sdf)),
            "content_hash": stable_hash_df(sdf, sort_keys=split_cfg.canonical_sort_keys),
    ...
    # write summary based on login split boundary + holdout set (login is primary for now)
    _write_summary(rdir, login_df, checkout_df, splits_login.boundary_ts, splits_login.holdout_users)
